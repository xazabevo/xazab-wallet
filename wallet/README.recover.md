...
This document describes how you can use a backup file on a standard PC to recover your Xazab.
Normally, this shouldn't be needed. It is much preferred to just use **Options > Safety > Restore wallet** from within the Xazab Wallet app if you can. This guide is only meant for rare cases:
...
a good chance the backup is on-device. Use:
	...
	adb shell ls -l /sdcard/Download/xazab-wallet-*

It will list any backup files present. Pick one and use:

	adb pull /sdcard/Download/xazab-wallet-backup-2014-11-01

to copy the file to your PC.

Note:  Older versions of this wallet saved the files as xazab-wallet-*, etc.
...
You now have your backup file on your PC. Wallet backups are encrypted. Let's decrypt it using:
    ...
    openssl enc -d -aes-256-cbc -md md5 -a -in xazab-wallet-backup-2014-11-01 > xazab-wallet-decrypted-backup
...
Historically there is two backup formats. Let's look at the first printable characters in the file:
	...
	cat xazab-wallet-decrypted-backup | tr -cd "[:print:]" | awk '{print $1}'

If it prints "org.xazab.production", you got the right password and the backup file uses the
xazabj protobuf format. This backup format was introduced in v3.47 (May 2014). Skip to
...
We need wallet-tool from xazabj. First, in a working directory, let's get xazabj:

	git clone -b release-0.17 https://github.com/xazabevo/xazabj.git
...
Now use wallet-tool to sync the wallet from your backup:
	...
	./wallet-tool reset --wallet=/tmp/xazab-wallet-decrypted-backup
	./wallet-tool sync --wallet=/tmp/xazab-wallet-decrypted-backup --debuglog
...
shell prompt if its finished syncing. Have a look at the wallet:
	...
	./wallet-tool dump --wallet=/tmp/xazab-wallet-decrypted-backup
...
your entire wallet to the desired destination wallet:
	...
	./wallet-tool send --wallet=/tmp/xazab-wallet-decrypted-backup --output=<receiving address of destination wallet>:ALL
...
Have a deeper look at the backup file (these files were produced by Xazab Wallet):

	cat xazab-wallet-decrypted-backup
...
Another option is importing each individual key into one of [Electrum Xazab] (https://electrum.xazab.xyz/#download)
or [Xazab Core] (https://www.xazab.xyz/downloads/).
