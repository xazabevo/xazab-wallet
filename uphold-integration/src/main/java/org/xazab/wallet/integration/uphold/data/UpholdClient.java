...
public class UpholdClient {
    ...
    private void getCards(final Callback<String> callback, final Callback<UpholdCard> getXazabCardCb) {
        service.getCards().enqueue(new retrofit2.Callback<List<UpholdCard>>() {
            ...
            public void onResponse(Call<List<UpholdCard>> call, Response<List<UpholdCard>> response) {
                if (response.isSuccessful()) {
                    ...
                    xazabCard = getXazabCard(response.body());
                    if (xazabCard == null) {
                        log.info("Xazab Card not available");
                        createXazabCard(callback, getXazabCardCb);
                    } else {
                        if (xazabCard.getAddress().getCryptoAddress() == null) {
                            log.info("Xazab Card has no addresses");
                            createXazabAddress(xazabCard.getId());
                        }
                        callback.onSuccess(xazabCard.getId());
                        if (getXazabCardCb != null) {
                            getXazabCardCb.onSuccess(xazabCard);
                        }
                    }
                } else {
    ...
    private void createXazabCard(final Callback<String> callback, final Callback<UpholdCard> getXazabCardCb) {
        Map<String, String> body = new HashMap<>();
        body.put("label", "Xazab Card");
        body.put("currency", "XAZAB");
        service.createCard(body).enqueue(new retrofit2.Callback<UpholdCard>() {
            ...
            public void onResponse(Call<UpholdCard> call, Response<UpholdCard> response) {
                if (response.isSuccessful()) {
                    log.info("Xazab Card created successfully");
                    xazabCard = response.body();
                    String xazabCardId = xazabCard.getId();
                    callback.onSuccess(xazabCardId);
                    createXazabAddress(xazabCardId);
                    if (getXazabCardCb != null) {
                        getXazabCardCb.onSuccess(response.body());
                    }
                } else {
                    log.error("Error creating Xazab Card: " + response.message() + " code: " + response.code());
                    callback.onError(new UpholdException("Error creating Xazab Card", response.message(), response.code()), false);
                }
            }

            @Override
            public void onFailure(Call<UpholdCard> call, Throwable t) {
                log.error("Error creating Xazab Card " + t.getMessage());
            }
        });
    }

    private void createXazabAddress(String cardId) {
        Map<String, String> body = new HashMap<>();
        body.put("network", "xazab");
        service.createCardAddress(cardId, body).enqueue(new retrofit2.Callback<UpholdCryptoCardAddress>() {
            ...
            public void onResponse(Call<UpholdCryptoCardAddress> call, Response<UpholdCryptoCardAddress> response) {
                log.info("Xazab Card address created");
            }

            @Override
            public void onFailure(Call<UpholdCryptoCardAddress> call, Throwable t) {
                log.error("Error creating Xazab Card address: " + t.getMessage());
            }
        });
    }

    private UpholdCard getXazabCard(List<UpholdCard> cards) {
        for (UpholdCard card : cards) {
            if(card.getCurrency().equalsIgnoreCase("xazab")) {
                return card;
            }
        }
        return null;
    ...
    public void getXazabBalance(final Callback<BigDecimal> callback) {
        getCards(new Callback<String>() {
            ...
            public void onError(Exception e, boolean otpRequired) {
                log.error("Error loading Xazab balance: " + e.getMessage());
    ...
    public void createXazabWithdrawalTransaction(String amount, String address,
        ...
        denomination.put("currency", "XAZAB");
        body.put("denomination", denomination);
        body.put("destination", address);

        service.createTransaction(xazabCard.getId(), body).enqueue(new retrofit2.Callback<UpholdTransaction>() {
            @Override
    ...
    public void commitTransaction(String txId, final Callback<Object> callback) {
        service.commitTransaction(xazabCard.getId(), txId).enqueue(new retrofit2.Callback<Object>() {
            @Override
    ...
    public UpholdCard getCurrentXazabCard() {
        return xazabCard;
    }
